# coding: utf-8

"""
    Orders

    An order represents a payment between two or more parties. Use the Orders API to create, update, retrieve, authorize, and capture orders.

    The version of the OpenAPI document: 2.13
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel, ConfigDict


class TrackerItem(BaseModel):
    # The item name or title.
    name: typing.Optional[str] = Field(None, alias='name')

    # The item quantity. Must be a whole number.
    quantity: typing.Optional[str] = Field(None, alias='quantity')

    # The stock keeping unit (SKU) for the item. This can contain unicode characters.
    sku: typing.Optional[str] = Field(None, alias='sku')

    # The URL to the item being purchased. Visible to buyer and used in buyer experiences.
    url: typing.Optional[str] = Field(None, alias='url')

    # The URL of the item's image. File type and size restrictions apply. An image that violates these restrictions will not be honored.
    image_url: typing.Optional[str] = Field(None, alias='image_url')

    upc: typing.Optional[typing.Union[bool, date, datetime, dict, float, int, list, str, None]] = Field(None, alias='upc')

    model_config = ConfigDict(
        protected_namespaces=(),
        arbitrary_types_allowed=True
    )
